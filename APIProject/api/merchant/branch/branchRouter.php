...
if ($request == "get") {
    @$branch_details = new BranchSystem\branch_details($db01);
    @$responseData = $branch_details->getBranchsInfo($auth['merchant_id_admin'], $auth['merchant_id']);
} elseif ($request == "new") {
    @$userupdate_class = new BranchSystem\branch_details($db01);
    @$responseData = $userupdate_class->branch_add_details(sanitize_input_data($_POST, $db01), $auth['merchant_id_admin'], $auth['merchant_id']);
} elseif ($request == "edit") {
    @$responseData = "under dev.";
} elseif ($request == "delete") {
    @$branch_delete_class = new BranchSystem\branch_details($db01);
    @$responseData = $branch_delete_class->branch_delete_details(sanitize_input_data($_POST, $db01), $auth['merchant_id_admin'], $auth['merchant_id']);
} else {
