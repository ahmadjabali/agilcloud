...
if (isset($_POST['email']) and isset($_POST['password'])) {
    ...
    $sql        = "SELECT * FROM agil_admin WHERE email = '" . $user1 . "'";
    ...
    if ($count == 1) {
        ...
        if (password_verify($mypassword, $row['pkhash'])) {
            unset($row['pkhash']);
            if ($row['op'] == ':=') {
                $userid = $row['id'];

                $update0 = mysqli_query($db01, "UPDATE agil_admin SET last_ip_address='" . get_client_ip() . "'  WHERE email = '$user1' LIMIT 1 ") or die("MError0001");
